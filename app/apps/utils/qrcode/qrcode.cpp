...
{
    // Get module
    const QrCode qr = QrCode::encodeText(text, QrCode::Ecc::ECC_LOW);
    ...
    for (int qr_y = 0; qr_y < qr.getSize(); qr_y++) {
        for (int qr_x = 0; qr_x < qr.getSize(); qr_x++) {
            ...
            HAL::GetCanvas()->fillRect(qr_image_cursor.x, qr_image_cursor.y, scale, scale,
                                       qr.getModule(qr_x, qr_y) ? colorOn : colorOff);
...
void QRCODE::RenderQRCodeBitmap(std::vector<std::vector<bool>>& bitmap, int x, int y, int size, uint32_t colorOn,
                                uint32_t colorOff)
{
    // Get scale
