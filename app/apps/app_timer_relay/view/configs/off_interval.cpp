...
void ConfigOffInterval::onInit()
{
}

void ConfigOffInterval::onPopOut()
{
...
void ConfigOffInterval::onHide()
{
}

void ConfigOffInterval::onRender()
{
    if (isHidden()) return;
    ...
    HAL::GetCanvas()->drawString("OFF Interval:", frame.x + 10, frame.y + _panel_h / 2 - 1);

    // Value
    HAL::GetCanvas()->setTextDatum(middle_right);
    if (_config->offInterval >= (1000 * 10 * 6)) {
        _string_buffer = fmt::format("{}min", _config->offInterval / (1000 * 10 * 6));
    } else if (_config->offInterval >= 1000) {
        _string_buffer = fmt::format("{}s", _config->offInterval / 1000);
    } else {
        _string_buffer = fmt::format("{:.1f}s", (float)_config->offInterval / 1000);
    }
    HAL::GetCanvas()->drawString(_string_buffer.c_str(), frame.x + _panel_w - 10, frame.y + _panel_h / 2 - 1);
}

void ConfigOffInterval::onUpdate()
{
}

static std::vector<std::uint32_t> _off_interval_option_list = {100,
                                                               200,
                                                               500,
                                                               800,
                                                               1000,
...
void ConfigOffInterval::onSelect()
{
    // Create select menu
    SelectMenuPage::CreateAndWaitResult([&](SelectMenuPage::Props_t& props) {
        props.primary                  = _color_panel;
        props.onPrimary                = _color_text;
        props.title                    = "OFF Interval:";
        props.onCustomRenderBackground = []() { ConfigPanel::RenderConfigPanel(); };
        props.onOptionSelected         = [&](int selectedIndex) {
            // Apply
            _config->offInterval = _off_interval_option_list[selectedIndex];
        };

        // Option list
        props.optionList.resize(_off_interval_option_list.size());
        for (int i = 0; i < _off_interval_option_list.size(); i++) {
            if (_off_interval_option_list[i] >= 1000 * 10 * 6) {
                _string_buffer = fmt::format("{} min", _off_interval_option_list[i] / (1000 * 10 * 6));
            } else if (_off_interval_option_list[i] >= 1000) {
                _string_buffer = fmt::format("{} s", _off_interval_option_list[i] / 1000);
            } else {
                _string_buffer = fmt::format("{:.1f} s", (float)_off_interval_option_list[i] / 1000);
            }
        ...
        // Startup index
        for (int i = 0; i < _off_interval_option_list.size(); i++) {
            if (_config->offInterval == _off_interval_option_list[i]) {
...
void ConfigOffInterval::onUnSelect()
{
}
