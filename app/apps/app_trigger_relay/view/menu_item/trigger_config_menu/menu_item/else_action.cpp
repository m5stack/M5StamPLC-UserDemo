...
void MenuItem_ElseAction::onInit()
{
}

void MenuItem_ElseAction::onPopOut()
{
...
void MenuItem_ElseAction::onHide()
{
}

void MenuItem_ElseAction::onRender()
{
    if (isHidden()) {
        return;
    }
    ...
    HAL::GetCanvas()->drawString("Else:", frame.x + 8, frame.y + _panel_h / 2 - 1);

    HAL::GetCanvas()->setTextDatum(middle_right);
    HAL::GetCanvas()->drawString(TriggerRelay::GetActionDescription(_trigger->onFalseAction), frame.x + _panel_w - 8,
                                 frame.y + _panel_h / 2 - 1);
}

void MenuItem_ElseAction::onUpdate()
{
}

void MenuItem_ElseAction::onSelect()
{
    // Create select menu
    SelectMenuPage::CreateAndWaitResult([&](SelectMenuPage::Props_t& props) {
        ...
        props.onOptionSelected         = [&](int selectedIndex) {
            // Apply
            _trigger->onFalseAction = TriggerRelay::GetActionList()[selectedIndex];
        };
        ...
        // Startup index
        for (int i = 0; i < TriggerRelay::GetActionList().size(); i++) {
            if (_trigger->onFalseAction == TriggerRelay::GetActionList()[i]) {
...
void MenuItem_ElseAction::onUnSelect()
{
}
